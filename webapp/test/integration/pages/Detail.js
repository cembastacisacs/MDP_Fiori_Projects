				...
				theLineItemsListShouldHaveTheCorrectNumberOfItems: function () {
					return this.waitFor({
						...
						check: function (oList) {
							var aEntitySet = this.getEntitySet("to_adrc");
							var sObjectID = oList.getBindingContext().getProperty("soldTo");
							var iLength = aEntitySet.filter(function (oLineItem) {
								return oLineItem.soldTo === sObjectID;
							}).length;
					...
					return this.waitFor({
						...
						success: function (oList) {
							...
							return this.waitFor({
								...
								matchers: new PropertyStrictEquals({
									name: "text",
									value: "<to_adrcPlural> (" + iNumberOfItems + ")"
								}),
								success: function () {
